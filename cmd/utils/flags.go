...
// Package utils contains internal helper functions for abey commands.
package utils
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		...
		Value: abey.DefaultConfig.NetworkId,
	}
	...
	BFTPortFlag = cli.Uint64Flag{
		...
		Value: uint64(abey.DefaultConfig.Port),
	}
	BFTStandbyPortFlag = cli.Uint64Flag{
		Name:  "bftport2",
		Usage: "committee node standby port ",
		Value: uint64(abey.DefaultConfig.StandbyPort),
	}
	...
	LightPeersFlag = cli.IntFlag{
		...
		Value: abey.DefaultConfig.LightPeers,
	}
	...
	TxPoolPriceLimitFlag = cli.Uint64Flag{
		...
		Value: abey.DefaultConfig.TxPool.PriceLimit,
	}
	TxPoolPriceBumpFlag = cli.Uint64Flag{
		Name:  "txpool.pricebump",
		Usage: "Price bump percentage to replace an already existing transaction",
		Value: abey.DefaultConfig.TxPool.PriceBump,
	}
	TxPoolAccountSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.accountslots",
		Usage: "Minimum number of executable transaction slots guaranteed per account",
		Value: abey.DefaultConfig.TxPool.AccountSlots,
	}
	TxPoolGlobalSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.globalslots",
		Usage: "Maximum number of executable transaction slots for all accounts",
		Value: abey.DefaultConfig.TxPool.GlobalSlots,
	}
	TxPoolAccountQueueFlag = cli.Uint64Flag{
		Name:  "txpool.accountqueue",
		Usage: "Maximum number of non-executable transaction slots permitted per account",
		Value: abey.DefaultConfig.TxPool.AccountQueue,
	}
	TxPoolGlobalQueueFlag = cli.Uint64Flag{
		Name:  "txpool.globalqueue",
		Usage: "Maximum number of non-executable transaction slots for all accounts",
		Value: abey.DefaultConfig.TxPool.GlobalQueue,
	}
	TxPoolLifetimeFlag = cli.DurationFlag{
		Name:  "txpool.lifetime",
		Usage: "Maximum amount of time non-executable transaction are queued",
		Value: abey.DefaultConfig.TxPool.Lifetime,
	}
	//fruit pool settings
	...
	GasTargetFlag = cli.Uint64Flag{
		...
		Value: abey.DefaultConfig.MinerGasFloor,
	}

	GasLimitFlag = cli.Uint64Flag{
		Name:  "gaslimit",
		Usage: "Target gas ceiling for fast block",
		Value: abey.DefaultConfig.MinerGasCeil,
	}
	...
	GasPriceFlag = BigFlag{
		...
		Value: abey.DefaultConfig.GasPrice,
	}
	...
	AbeystatsURLFlag = cli.StringFlag{
		Name:  "abeystats",
		Usage: "Reporting URL of a abeystats service (nodename:secret@host:port)",
	}
	...
	GpoBlocksFlag = cli.IntFlag{
		...
		Value: abey.DefaultConfig.GPO.Blocks,
	}
	GpoPercentileFlag = cli.IntFlag{
		Name:  "gpopercentile",
		Usage: "Suggested gas price is the given percentile of a set of recent transaction gas prices",
		Value: abey.DefaultConfig.GPO.Percentile,
	}
...
// for Gabey and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses!(can search via `gabey account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func setEtherbase(ctx *cli.Context, ks *keystore.KeyStore, cfg *abey.Config) {
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "ABEY", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
func setEthash(ctx *cli.Context, cfg *abey.Config) {
...
// RegisterAbeyService adds an Abeychain client to the stack.
func RegisterAbeyService(stack *node.Node, cfg *abey.Config) {
	var err error
...
func RegisterAbeystatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both abey and les services
		var abeyServ *abey.Abeychain
		ctx.Service(&abeyServ)

		var lesServ *les.LightAbey
		ctx.Service(&lesServ)

		return abeystats.New(url, abeyServ, lesServ)
	}); err != nil {
		Fatalf("Failed to register the Abeychain Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gabey.", map[string]string{
				"host": hosttag,
			})
		}
	}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) abeydb.Database {
	var (
...
func MakeChain(ctx *cli.Context, stack *node.Node) (fchain *core.BlockChain, schain *snailchain.SnailBlockChain, chainDb abeydb.Database) {
	var err error
	...
	if !ctx.GlobalBool(FakePoWFlag.Name) {
		engine = minerva.New(minerva.Config{
			CacheDir:       stack.ResolvePath(abey.DefaultConfig.MinervaHash.CacheDir),
			CachesInMem:    abey.DefaultConfig.MinervaHash.CachesInMem,
			CachesOnDisk:   abey.DefaultConfig.MinervaHash.CachesOnDisk,
			DatasetDir:     stack.ResolvePath(abey.DefaultConfig.MinervaHash.DatasetDir),
			DatasetsInMem:  abey.DefaultConfig.MinervaHash.DatasetsInMem,
			DatasetsOnDisk: abey.DefaultConfig.MinervaHash.DatasetsOnDisk,
		})
	}
	if gcmode := ctx.GlobalString(GCModeFlag.Name); gcmode != "full" && gcmode != "archive" {
		Fatalf("--%s must be either 'full' or 'archive'", GCModeFlag.Name)
...
// new format.
//
// e.g. gabey account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gabey --keystore /tmp/mykeystore --lightkdf account new
//
