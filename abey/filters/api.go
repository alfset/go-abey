...
// `abey_getFilterChanges` polling method that is also used for log filters.
//
// https://github.com/abey/wiki/wiki/JSON-RPC#abey_newpendingtransactionfilter
...
// It is part of the filter package since polling goes with abey_getFilterChanges.
//
// https://github.com/ethereum/wiki/wiki/JSON-RPC#abey_newblockfilter
...
func (api *PublicFilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(abeychain.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
type FilterCriteria abeychain.FilterQuery
...
//
// https://github.com/ethereum/wiki/wiki/JSON-RPC#abey_newfilter
func (api *PublicFilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	logs := make(chan []*types.Log)
	logsSub, err := api.events.SubscribeLogs(abeychain.FilterQuery(crit), logs)
	if err != nil {
		return rpc.ID(""), err
	}
...
//
// https://github.com/ethereum/wiki/wiki/JSON-RPC#abey_uninstallfilter
...
//
// https://github.com/ethereum/wiki/wiki/JSON-RPC#abey_getfilterlogs
...
//
// https://github.com/ethereum/wiki/wiki/JSON-RPC#abey_getfilterchanges
